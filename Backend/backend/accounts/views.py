...
class DevVerifyUserView(GenericAPIView):
    """
    DEV ONLY — manually verify a user by email without needing the email link.
    POST /api/accounts/auth/dev-verify/  { "email": "user@example.com" }
    ...
    def post(self, request):
        ...
        try:
            ...
            user.token_created_at = None
            user.save(update_fields=['is_verified', 'verification_token', 'token_created_at'])
            return Response({'message': f'{email} is now verified. You can log in.'})
